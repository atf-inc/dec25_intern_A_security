...
class PromptGuard:
    ...
    def _load_model_and_tokenizer(
        self,
        model_name: str = "meta-llama/Prompt-Guard-86M",  # Changed to use lighter Prompt-Guard-86M
        ...
        model_path = os.path.expanduser(
            os.path.join(os.environ["HF_HOME"], "hub", f"models--{model_name.replace('/', '--')}")
        )
        
        # Also check standard HF cache location
        standard_cache_path = os.path.expanduser(
            os.path.join("~/.cache/huggingface/hub", f"models--{model_name.replace('/', '--')}")
        )
        standard_cache_path = os.path.expanduser(standard_cache_path)

        # Check if the NEW model (Prompt-Guard-86M) exists first
        ...
        if os.path.exists(model_path):
            ...
            if os.path.exists(old_model_path):
                LOG.info(f"Found old model at {old_model_path}, but using new model: {model_name}")
            LOG.warning(
