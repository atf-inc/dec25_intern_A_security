...
export async function middleware(request: NextRequest) {
  ...
  if (WAF_ENABLED) {
    // Avoid infinite loop: Don't check requests to the WAF API itself
    if (request.nextUrl.pathname.startsWith('/api/waf/')) {
    ...
    try {
      ...
      console.log(`[WAF] Calling WAF API: ${WAF_API_URL}${WAF_API_ENDPOINT}`)
      ...
      if (!result.allowed) {
        ...
        return NextResponse.json(
          {
            message: `ohh you are trying to attack , try again becouse there is shubham in between`,
            blocked: true,
            details: {
